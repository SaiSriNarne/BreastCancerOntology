...
/**
  * Created by Sai Sri on 11-10-2018.
  */
object OntContructorMain {
  def main(args: Array[String]): Unit = {

    val ONTOLOGYURI="http://www.semanticweb.org/saisri/ontologies/2018/10/"
    ...
    val classes=Source.fromFile("datanew/Classes").getLines()
    ...
    val subClasses=Source.fromFile("datanew/SubClasses").getLines()
    ...
    val objprop=Source.fromFile("datanew/ObjectProperties").getLines()
    objprop.foreach(f=> {
      val farr=f.split(",")
      ...
      //val objsome=df.getOWLSomeValuesFrom
    ...
    val dataprop=Source.fromFile("datanew/DataProperties").getLines()
    ...
    val individuals=Source.fromFile("datanew/Individuals").getLines()
    ...
    val triplets=Source.fromFile("datanew/Triplets").getLines()
    triplets.foreach(f=>{
      val farr=f.split(",")
    ...
    val os = new FileOutputStream("datanew/family.owl")
